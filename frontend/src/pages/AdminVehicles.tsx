...
export default function AdminVehicles() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 overflow-hidden relative isolate">
        <div className="absolute right-0 top-0 w-64 h-64 bg-slate-50 dark:bg-slate-950 rounded-full blur-3xl opacity-50 -translate-y-1/2 translate-x-1/3"></div>
        <div className="p-8 relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-slate-900 dark:text-slate-300 tracking-tight">Fleet Vehicles</h1>
              <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium">Manage your fleet and activate GPS trackers</p>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {/* Toolbar */}
        <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row gap-4 justify-between bg-slate-50/50 dark:bg-slate-950/50">
          <div className="relative max-w-md w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all bg-white dark:bg-slate-900"
            />
          </div>
          <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 font-medium">
            <Activity className="h-4 w-4" />
            Total Active Vehicles: <span className="text-slate-900 dark:text-slate-300 font-bold">{vehicles.length}</span>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="p-12 text-center text-slate-500 dark:text-slate-400">
              <Activity className="h-8 w-8 animate-spin mx-auto mb-3 text-slate-400 dark:text-slate-500" />
            ...
            <div className="p-16 text-center">
              <div className="inline-flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950 rounded-full mb-4">
                <Car className="h-8 w-8 text-slate-400 dark:text-slate-500" />
              </div>
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-300 mb-2">No Vehicles Found</h3>
              <p className="text-slate-500 dark:text-slate-400 max-w-sm mx-auto mb-6">You don't have any vehicles matching your search, or you haven't activated any trackers yet.</p>
              <button onClick={openAddModal} className="text-slate-900 dark:text-slate-300 font-semibold hover:underline">
            ...
            <div style={{ height: "600px" }}>
              <TableVirtuoso
                ...
                fixedHeaderContent={() => (
                  <tr className="bg-slate-50 dark:bg-slate-950/80 border-b border-slate-200">
                    <th className="font-semibold text-slate-600 dark:text-slate-400 text-sm py-4 px-6 text-left">Vehicle Details</th>
                    <th className="font-semibold text-slate-600 dark:text-slate-400 text-sm py-4 px-6 text-left">License Plate</th>
                    <th className="font-semibold text-slate-600 dark:text-slate-400 text-sm py-4 px-6 text-left">Tracker Connection</th>
                    <th className="font-semibold text-slate-600 dark:text-slate-400 text-sm py-4 px-6 text-right">Actions</th>
                  </tr>
                )}
                itemContent={(_index, vehicle) => (
                  <>
                    <td className="py-4 px-6 border-b border-slate-100">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 bg-slate-100 rounded-lg flex items-center justify-center text-slate-500 dark:text-slate-400">
                          <Car className="h-5 w-5" />
                        </div>
                        <div>
                          <div className="font-semibold text-slate-900 dark:text-slate-300">{vehicle.name}</div>
                          <div className="text-xs text-slate-500 dark:text-slate-400">ID: {vehicle.id}</div>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-6 border-b border-slate-100">
                      <div className="inline-flex items-center px-2.5 py-1 rounded-md bg-slate-100 border border-slate-200 text-sm font-mono font-bold text-slate-700 dark:text-slate-300">
                    ...
                    <td className="py-4 px-6 text-right border-b border-slate-100">
                      <div className="flex items-center justify-end gap-2">
                        <RouterLink
                          ...
                          className="p-2 text-slate-400 dark:text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors border border-transparent hover:border-blue-100 flex items-center gap-1.5"
                        ...
                        <button
                          ...
                          className="p-2 text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:text-slate-300 hover:bg-slate-100 rounded-lg transition-colors border border-transparent hover:border-slate-200"
                          title="Edit Vehicle"
                        >
                        ...
                        <button
                          ...
                          className="p-2 text-slate-400 dark:text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-transparent hover:border-red-100"
      ...
      >
        <form onSubmit={handleSaveVehicle} className="space-y-6">
          <div className="space-y-4">
            ...
            {!isEditModalOpen && (
              <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 rounded-lg p-4 flex gap-4">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-300">Link your hardware</h4>
            ...
            <div className="border-t border-slate-200 pt-4 mt-4">
              <label className="flex items-center gap-2 text-sm font-medium text-slate-900 dark:text-slate-300 mb-1">
              ...
              <div className="relative">
                <Cpu className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 dark:text-slate-500" />
                <input
                  type="text"
              ...
              <div className="pt-2">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Assigned Route (Optional)</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
                    <Link className="h-5 w-5 text-slate-400 dark:text-slate-500" />
                  </div>
                  <select
                    className="w-full pl-10 pr-4 py-3 border-2 border-slate-200 rounded-lg focus:ring-0 focus:border-slate-900 transition-colors bg-white dark:bg-slate-900 font-medium text-slate-700 dark:text-slate-300 appearance-none"
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
            <button
              type="button"
              onClick={closeModals}
              className="px-5 py-2.5 text-slate-600 dark:text-slate-400 hover:bg-slate-100 rounded-lg font-medium transition-colors"
            >
              Cancel
            </button>
            <button
