...
const VehicleHistory = () => {
  ...
  return (
    <div className="flex h-full bg-white relative overflow-hidden">
      ...
      <div className="w-80 bg-white border-r border-slate-200 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-[1000] relative">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative mt-1">
              <button
                ...
                className="w-full flex items-center justify-between pl-3 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm transition-all shadow-sm text-left"
              >
                <div className="flex items-center gap-2">
                  <Car className="h-4 w-4 text-slate-400" />
                  <span className={cn("font-medium", selectedVehicle ? "text-slate-900" : "text-slate-400")}>
                    {selectedVehicle 
                      ? vehicles.find(v => v.id === selectedVehicle)?.name || 'Unknown'
                      : 'Select a vehicle'}
                  </span>
                </div>
                <ChevronDown className={cn("h-4 w-4 text-slate-400 transition-transform duration-200", isSelectOpen && "rotate-180")} />
              </button>

              {isSelectOpen && (
                <>
                  <div className="fixed inset-0 z-[1999]" onClick={() => { setIsSelectOpen(false); setVehicleSearch(''); }} />
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-xl shadow-2xl z-[2000] max-h-[300px] flex flex-col overflow-hidden">
                    <div className="p-2 border-b border-slate-100 bg-slate-50/50">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full bg-white border border-slate-200 rounded-lg pl-8 pr-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-700 font-medium"
                        />
                      </div>
                    </div>
                        ...
                        vehicles.filter(v => 
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              selectedVehicle === v.id ? "bg-blue-50/50 text-blue-700" : "hover:bg-slate-50 text-slate-700"
                            )}
                          >
                            <div className="flex flex-col">
                              <span className="font-bold">{v.name}</span>
                              <span className="text-[10px] text-slate-400 font-mono tracking-wider uppercase mt-0.5">{v.licensePlate}</span>
                            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className={cn(
                    "block w-full pl-9 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm transition-all shadow-sm outline-none cursor-pointer",
                    fromDate ? "text-slate-900 font-medium" : "text-slate-400"
                  )} 
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className={cn(
                    "block w-full pl-9 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm transition-all shadow-sm outline-none cursor-pointer",
                    toDate ? "text-slate-900 font-medium" : "text-slate-400"
                  )} 
                />
              </div>
            </div>
